...
function PropertyCard({ contact, onSelect }) {
  const { data: property, isLoading } = useQuery({
    queryKey: ['owner-property', contact.supabaseid],
    queryFn: () => getProperty(contact.supabaseid),
    enabled: !!contact.supabaseid,
  });

  const completion = property ? computeCompletion(property) : 0;
  const name = property?.name?.fr || property?.name || contact.contactname || contact.supabase_id;
    ...
    <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5 flex flex-col gap-3 hover:shadow-md transition-shadow">
      ...
      <div>
        <div className="flex items-center justify-between text-xs text-gray-500 mb-1">
          ...
          <span className="font-semibold" style={{ color: completion >= 80 ? '#16a34a' : '#8B1A1A' }}>{completion}%</span>
        </div>
  ...
  return (
    <div className="min-h-screen pb-8" style={{ background: '#f9f6f3' }}>
      ...
      <div className="max-w-2xl mx-auto px-4 pt-6">
        ...
        <p className="text-sm text-gray-500 mb-5">À quelle propriété souhaitez-vous accéder ?</p>

        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {contacts.map(contact => (
            <PropertyCard
              key={contact.supabaseid}
              contact={contact}
              onSelect={onSelect}
            />
          ))}
