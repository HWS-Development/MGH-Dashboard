<?php

use Illuminate\Support\Facades\Route;

/*
|--------------------------------------------------------------------------
| Web Routes
|--------------------------------------------------------------------------
|
| The SPA (React) handles all frontend routing. In development, Vite serves
| the frontend directly. In production, the catch-all serves index.html.
|
*/

// Handle static assets and serve the SPA fallback.
// Exclude: /api, /sanctum, /_ignition, /up (health check)
Route::get('/{any?}', function (string $any = '') {
    // Check if the file exists in dist/ (served with correct MIME).
    $distFile = base_path('dist/' . $any);
    if ($any !== '' && file_exists($distFile) && is_file($distFile)) {
        $mime = match (pathinfo($distFile, PATHINFO_EXTENSION)) {
            'js'   => 'application/javascript',
            'css'  => 'text/css',
            'json' => 'application/json',
            'svg'  => 'image/svg+xml',
            'webp' => 'image/webp',
            'png'  => 'image/png',
            'jpg',
            'jpeg' => 'image/jpeg',
            'ico'  => 'image/x-icon',
            'woff2'=> 'font/woff2',
            'woff' => 'font/woff',
            'ttf'  => 'font/ttf',
            default => mime_content_type($distFile) ?: 'application/octet-stream',
        };
        return response(file_get_contents($distFile), 200, ['Content-Type' => $mime]);
    }

    // SPA fallback: serve dist/index.html for any non-API route.
    $indexPath = base_path('dist/index.html');
    if (!file_exists($indexPath)) {
        return response()->json(['error' => 'Not found'], 404);
    }
    return response(file_get_contents($indexPath), 200, ['Content-Type' => 'text/html']);
})->where('any', '^(?!api|sanctum|_ignition|up).*$')->name('spa');
